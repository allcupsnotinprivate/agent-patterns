...
ReAct - практический **дефолт для агента с инструментами**. В современных SDK он реализуется не парсингом текста, а нативным [function/tool calling](../../02-tool-use/function-calling/): «Thought» - это текстовый блок перед вызовом, «Action» - блок `tool_use`, «Observation» - блок `tool_result`, отправляемый обратно.
