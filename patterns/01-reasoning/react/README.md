...
- инструментов много и стоимость критична - рассмотрите **ReWOO** или **Plan-and-Execute** (меньше LLM-вызовов);
...
- **«Близорукость».** Классический ReAct планирует на один шаг вперед и не делает бэктрекинг - при плохом раннем выборе может «застрять». Деревья (**ToT**, **LATS**) это лечат ценой резкого роста вычислений.
...
- **ReWOO** - отделяет план от наблюдений: план с плейсхолдерами строится заранее, инструменты исполняются пакетно, поздняя интеграция; всего 2 LLM-вызова.
- **Plan-and-Execute** - план всех шагов заранее, затем исполнение (часто дешевой моделью), реплан при сбое.
- **Reflexion** - надстройка над ReAct: после неудачной попытки агент пишет саморефлексию в эпизодическую память и учитывает ее в следующей попытке.
- **LATS** - поиск по дереву (MCTS) поверх ReAct-траекторий с value-функциями и рефлексией.
