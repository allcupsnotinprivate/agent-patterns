...
CoT почти ничего не стоит (один вызов, чуть больше выходных токенов) и часто идет «фоном» в других паттернах: [ReAct](../react/) добавляет к рассуждению действия, [Self-Consistency](../self-consistency/) берет несколько CoT-траекторий и голосует, деревья ([ToT](../tot/)) ветвят их.
...
- **Точность vs линейность.** Прирост качества реален, но одна траектория не защищает от ранней ошибки. Лечится выборкой нескольких путей ([Self-Consistency](../self-consistency/)) или деревьями ([ToT](../tot/)) - ценой ×N вызовов.
